...
- Use ADK stream events for observability. A run's stream carries every event its invocations append to the session, including those of nested runs started with `ctx.run`, `spawn` or `dispatch`, plus text and thought deltas. `onEvent` hooks receive exactly the stream's events, in the same order. If a run yields or fails while spawned or dispatched work is still running, its hooks also receive that work's events until it finishes, although its stream has ended; an aborted run's hooks receive nothing further. A `state_change` belongs to a run when the run's own invocations recorded it, including its nested, spawned and dispatched work. Once its main work settles, a run keeps only the spawns and dispatches still open, so a later run that resumes the same invocation receives its own changes. A direct `session.state` write outside any invocation belongs to no run. The stream includes `state_change` for every scope, so drop events for which `mayLeaveProcess(event)` returns `false` before forwarding them outside the process. `model_start` carries the step index, message count, tool names and descriptions, and output schema. `model_end` carries usage, provider-reported cost, duration, and finish reason. Neither carries the rendered context.
