...
That's the answer.`
  ...
  describe('createParser', () => {
    it('should create a reusable parser', () => {
      ...
      const result1 = parser.parse('{"name": "John", "age": 30, "role": "user"}')
      expect(result1.success).toBe(true)

      const result2 = parser.parse('{"name": "Jane", "age": 25, "role": "admin"}')
    ...
    it('should support configuration', () => {
      ...
      const result = parser.parse('{"name": "John", "age": "30", "role": "user"}')
      expect(result.success).toBe(false)
    })
  })
})
...
describe('JSON Repair Features via parseJsonish', () => {
  describe('comment removal', () => {
    ...
    it('should not remove comments inside strings', () => {
      const input = '{"message": "// not a comment", "url": "https://example.com"}'
      const result = parseJsonish(input)
      expect(result.success).toBe(true)
      expect(result.value).toEqual({
...
describe('Deeply Nested Structures', () => {
  ...
  it('should track nested correction paths accurately', () => {
    ...
    expect(result.success).toBe(true)
    if (result.success) {
      const ageCorrection = result.corrections.find((c) => c.path.join('.') === 'user.profile.age')
...
describe('Streaming Delta Computation', () => {
  it('should detect string append operations', () => {
    ...
    const appendDelta = r2.delta?.find((d) => d.operation === 'append' || d.operation === 'set')
...
describe('Circular reference detection', () => {
  ...
  it('should allow identical values for fields with same object shape', () => {
    const schema = z.object({
      home_address: addressShape,
      work_address: addressShape,
    })
    const value = {
      home_address: { street: '123 Main', city: 'NYC' },
      work_address: { street: '123 Main', city: 'NYC' },
    }
    const result = coerce(value, schema)
    expect(result.success).toBe(true)
    if (result.success) {
      expect(result.value.home_address).toEqual(value.home_address)
      expect(result.value.work_address).toEqual(value.work_address)
    }
  })

  it('should parse recursive z.lazy schema with valid nested structure', () => {
    ...
    const result = coerce(value, treeSchema)
    expect(result.success).toBe(true)
    if (result.success) {
      expect(result.value.value).toBe(1)
      expect(result.value.children).toHaveLength(2)
      expect(result.value.children[0].value).toBe(2)
      expect(result.value.children[1].children[0].value).toBe(4)
    }
  })
})
