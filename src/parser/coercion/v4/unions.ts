...
export function coerceDiscriminatedUnion(
  ...
  const optionsMap = new Map<unknown, z.core.$ZodType>()
  for (const option of schema.options) {
    for (const tag of option._zod.propValues?.[discriminator] ?? []) {
      optionsMap.set(tag, option)
    }
  }
  let matchedSchema = optionsMap.get(discriminatorValue)

  if (!matchedSchema && discriminatorValue === undefined) {
  ...
  if (!(matchedSchema instanceof z.ZodType)) throw new Error('ADK schemas must use Zod Classic')
