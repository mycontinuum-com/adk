...
type MatchType = 'exact' | 'sanitized' | 'normalized' | 'stripped' | 'prefix' | 'substring'
...
export function coerceToEnum(
  ...
  if (typeof value !== 'string') {
    addError(ctx, `enum(${enumValues.join('|')})`, value, `Cannot coerce ${typeof value} to enum`)
    return undefined
  }
  ...
  if (match) {
    if (match.type !== 'exact') {
      ...
      addCorrection(ctx, value, match.value, correction.reason, correction.type)
    }
    return match.value
  }
