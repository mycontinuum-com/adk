...
function plainToJsonish(value: unknown, completionState: CompletionState): JsonishValue {
  ...
  if (typeof value === 'string') return { type: 'string', value, completionState }
  if (typeof value === 'number') return { type: 'number', value, completionState }
  if (typeof value === 'boolean') return { type: 'boolean', value, completionState }
...
export function parseJsonish(
  ...
  if (result.values.length > 0) {
    ...
    const jsonish: JsonishValue = fixes.length > 0 ? { type: 'fixed', value, fixes } : value
    return {
      success: true,
...
export function parsePartialJson(
  ...
  const { extractFromText = true } = options
  let content = extractFromText ? extractJsonFromText(text.trim()) : text.trim()
