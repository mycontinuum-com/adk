...
function collectionToValue(collection: CollectionType): JsonishValue | null {
  switch (collection.type) {
    ...
    case 'object': {
      ...
      for (let i = 0; i < collection.keys.length && i < collection.values.length; i++) {
...
function completeValueDeeply(value: JsonishValue): void {
  switch (value.type) {
    ...
    case 'boolean':
    case 'null':
      ;(value as { completionState: CompletionState }).completionState = 'complete'
      break
    case 'object':
...
function completeCollection(state: ParseState, completionState: CompletionState): void {
  ...
  if (last) {
    ...
    if (lastColl.type === 'object') {
      if (lastColl.keys.length === lastColl.values.length) {
        if (value.type === 'string' && typeof value.value === 'string') {
          ...
          lastColl.keys.push((value as { originalString: string }).originalString)
        } else {
    ...
    const name =
      collection.type === 'object' ? 'Object' : collection.type === 'array' ? 'Array' : 'string'
...
type Position = 'inNothing' | 'unknown' | 'inObjectKey' | 'inObjectValue' | 'inArray'

function getCurrentPosition(state: ParseState): Position {
  if (state.collectionStack.length < 2) return 'inNothing'

  const parent = state.collectionStack[state.collectionStack.length - 2].collection
  if (parent.type === 'object') {
    return parent.keys.length === parent.values.length ? 'inObjectKey' : 'inObjectValue'
  }
...
function isHex4At(str: string, index: number): boolean {
  ...
  for (let i = 0; i < 4; i++) {
    const c = str[index + i]
    const valid = (c >= '0' && c <= '9') || (c >= 'a' && c <= 'f') || (c >= 'A' && c <= 'F')
...
function shouldCloseUnquotedString(
  ...
  const pos = getCurrentPosition(state)
  const current = state.collectionStack[state.collectionStack.length - 1]?.collection
...
function findStartingValue(state: ParseState, str: string, index: number): number {
...
const handleTripleQuotedString: TokenHandler = (state, str, _index, char, afterIndex) => {
...
const handleQuotedString: TokenHandler = (state, str, _index, char, afterIndex) => {
  if (char === '"') {
...
const handleTripleBacktickString: TokenHandler = (state, str, _index, char, afterIndex) => {
  if (char === '`' && startsWithAt(str, afterIndex, '``')) {
    completeCollection(state, 'complete')
    return 2
  }
...
const handleBacktickString: TokenHandler = (state, str, _index, char, afterIndex) => {
  if (char === '`') {
    if (shouldCloseQuotedString(state, str, afterIndex, '`')) {
      completeCollection(state, 'complete')
      return 0
...
const handleSingleQuotedString: TokenHandler = (state, str, _index, char, afterIndex) => {
  if (char === "'") {
    if (shouldCloseQuotedString(state, str, afterIndex, "'")) {
      completeCollection(state, 'complete')
      return 0
...
const handleUnquotedString: TokenHandler = (state, str, _index, char, afterIndex) => {
  consumeChar(state, char)
...
const handleBlockComment: TokenHandler = (state, str, _index, char, afterIndex) => {
