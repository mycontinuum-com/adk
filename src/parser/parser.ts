...
export function createParser<T>(
  ...
  function parseWithCoercion(
    ...
    if (result.success) {
      return makeSuccessResult(result.value, raw, result.corrections, result.totalScore, jsonish)
    }
  ...
  function parseWithValidation(raw: unknown, jsonish?: JsonishValue): ParseResult<T> {
  ...
  return {
    ...
    parsePartial(input: string): ParseResult<Partial<T>> {
      ...
      return parseWithCoercion(jsonishResult.value, jsonishResult.jsonish, true)
    },
