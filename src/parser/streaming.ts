  ...
  switch (newValue.type) {
    ...
    case 'anyOf': {
      if (
        ...
        deltas.push(...computeDeltas<T>(oldValue.candidates[0], newValue.candidates[0], path))
      } else {
        deltas.push({
          path,
          value: jsonishToPlain(newValue),
  ...
  switch (value.type) {
    ...
    case 'array':
      for (let i = 0; i < value.items.length; i++) {
        getCompletionStatesRecursive(value.items[i], [...path, String(i)], states)
      }
      break
    case 'fixed':
...
export function parseStreamChunks<T>(schema: z.ZodType<T>, chunks: string[]): ParseResult<T> {
