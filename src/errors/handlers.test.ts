...
import { MockAdapter, createTestSession } from '../testing'
import {
...
describe('Error Handlers', () => {
  describe('composeErrorHandlers', () => {
    it('returns default recovery when no handlers', async () => {
      ...
      const result = await composed.handle(createErrorContext({ phase: 'tool' }))
      expect(result).toEqual({ action: 'skip' })
    })

    it('returns throw for model errors by default', async () => {
      const composed = composeErrorHandlers([], [])
      const result = await composed.handle(createErrorContext({ phase: 'model' }))
  ...
  describe('Integration', () => {
    it('agent errorHandlers are called during tool errors', async () => {
      ...
      const mockAdapter = new MockAdapter({
        responses: [{ toolCalls: [{ name: 'failing_tool', args: {} }] }, { text: 'Done' }],
      })
    ...
    it('runner errorHandlers wrap agent errorHandlers', async () => {
      ...
      const mockAdapter = new MockAdapter({
        responses: [{ toolCalls: [{ name: 'fail', args: {} }] }, { text: 'Done' }],
      })
    ...
    it('retry handler causes tool re-execution', async () => {
      ...
      const mockAdapter = new MockAdapter({
        responses: [{ toolCalls: [{ name: 'flaky', args: {} }] }, { text: 'Success!' }],
      })
