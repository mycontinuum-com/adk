...
import { agent, loop } from '../agents/factory'
import { adk } from '../api'
import { injectSystemMessage, includeHistory, buildContext } from '../context'
  ...
  describe('tool results from previous turns', () => {
    it('should include tool_call and tool_result events from previous turn in context', async () => {
      const testAgent = agent({
        ...
        context: [injectSystemMessage('You are a test assistant.'), includeHistory()],
        tools: [echoTool],
      })
      ...
      const capturedContext = buildContext(session, testAgent, 'test-invocation')
      ...
      const { contents } = serializeContext(capturedContext)
    ...
    it('should serialize multi-turn conversation with tool calls correctly', async () => {
      const testAgent = agent({
        name: 'test_agent',
        model: gemini('gemini-2.0-flash'),
        context: [injectSystemMessage('You are a calculator assistant.'), includeHistory()],
      ...
      firstTurnSerialized.contents.forEach((c: Content, i: number) => {
        ...
        c.parts?.forEach((p, j) => {
          if (p.text) console.log(`  Part ${j}: text = "${p.text.substring(0, 50)}..."`)
          if (p.functionCall) console.log(`  Part ${j}: functionCall = ${p.functionCall.name}`)
          if (p.functionResponse)
            console.log(`  Part ${j}: functionResponse = ${p.functionResponse.name}`)
        })
      })

      expect(firstTurnSerialized.contents.length).toBeGreaterThan(0)

      const hasCalculateFunctionCall = firstTurnSerialized.contents.some((c: Content) =>
        c.parts?.some((p) => p.functionCall?.name === 'calculate'),
      )
      const hasCalculateFunctionResponse = firstTurnSerialized.contents.some((c: Content) =>
    ...
    it('should preserve tool results across loop iterations', async () => {
      ...
      serialized.contents.forEach((c: Content, i: number) => {
        console.log(`[${i}] Role: ${c.role}`)
        c.parts?.forEach((p, j) => {
          if (p.text) console.log(`  Part ${j}: text = "${p.text.substring(0, 80)}..."`)
          if (p.functionCall)
            console.log(`  Part ${j}: functionCall = ${JSON.stringify(p.functionCall)}`)
          if (p.functionResponse)
            console.log(`  Part ${j}: functionResponse = ${JSON.stringify(p.functionResponse)}`)
        })
      })

      const toolResultEvents = firstIterContext.events.filter((e) => e.type === 'tool_result')
  ...
  describe('role grouping validation', () => {
    it('should maintain correct role sequence: user -> model (tool_call) -> user (tool_result) -> model (response)', async () => {
      ...
      expectedSequence.forEach((expected, i) => {
        ...
        if (expected.hasFunctionCall) expect(contentRoles[i]?.hasFunctionCall).toBe(true)
        if (expected.hasFunctionResponse) expect(contentRoles[i]?.hasFunctionResponse).toBe(true)
      })
    })
  })
  ...
  describe('consecutive tool calls', () => {
    it('should handle multiple consecutive tool calls and their results', async () => {
      const testAgent = agent({
        ...
        model: gemini('gemini-2.0-flash'),
        context: [injectSystemMessage('You can use multiple tools'), includeHistory()],
      ...
      serialized.contents.forEach((c: Content, i: number) => {
        ...
        c.parts?.forEach((p, j) => {
          if (p.functionCall) console.log(`  [${j}] functionCall: ${p.functionCall.name}`)
          if (p.functionResponse)
            console.log(`  [${j}] functionResponse: ${p.functionResponse.name}`)
        })
      })
      ...
      const toolCallCount = toolCallContent?.parts?.filter((p) => p.functionCall).length ?? 0
