...
describe('openai-endpoints', () => {
  describe('resolveModelName', () => {
    it('should return mapped name when mapping exists', () => {
      ...
      expect(resolveModelName('gpt-5-mini', endpoint)).toBe('gpt-5-mini-v2025-08-07')
    })
    ...
    it('should pass through unmapped model names', () => {
      ...
      expect(resolveModelName('custom-model-name', endpoint)).toBe('custom-model-name')
    })
    ...
    it('should pass through exact versioned names', () => {
      ...
      expect(resolveModelName('gpt-5-mini-2025-08-07', endpoint)).toBe('gpt-5-mini-2025-08-07')
    })
  })
  ...
  describe('getDefaultEndpoints', () => {
    ...
    it('should throw helpful error when no env vars set', () => {
      expect(() => getDefaultEndpoints()).toThrow('No OpenAI API key configured')
    ...
    it('should include model mappings in default endpoints', () => {
      ...
      const endpoints = getDefaultEndpoints()
      expect(endpoints[0].modelMapping?.['gpt-5-mini']).toBe('gpt-5-mini-v2025-08-07')
      expect(endpoints[1].modelMapping?.['gpt-5-mini']).toBe('gpt-5-mini-2025-08-07')
    })
  })

  describe('isRetryableForFallback', () => {
    it('should return true for rate limit errors', () => {
      expect(isRetryableForFallback(new Error('Rate limit exceeded'))).toBe(true)
    })
    ...
    it('should return true for 5xx errors', () => {
      expect(isRetryableForFallback(new Error('500 Internal Server Error'))).toBe(true)
      expect(isRetryableForFallback(new Error('502 Bad Gateway'))).toBe(true)
      expect(isRetryableForFallback(new Error('503 Service Unavailable'))).toBe(true)
    })

    it('should return true for connection errors', () => {
      expect(isRetryableForFallback(new Error('Connection refused'))).toBe(true)
    })

    it('should return true for model/deployment not found', () => {
      expect(isRetryableForFallback(new Error('Model not found'))).toBe(true)
      expect(isRetryableForFallback(new Error('Deployment not found'))).toBe(true)
    })
