export interface OpenAIEndpoint {
  ...
  /**
   * Opt out of the OpenAI client's browser guard, for a page where the END USER supplies their own
   * key (the guard's intended exception — it exists to stop a server key being embedded in shipped
   * client code). Never set this with a key the user did not type themselves.
   */
...
export function resolveModelName(logicalName: string, endpoint: OpenAIEndpoint): string {
