...
import { serializeContext, parseResponse, serializeTools, serializeToolConfig } from './gemini'
...
function getPart(contents: Content[], contentIdx: number, partIdx: number): Part {
  return contents[contentIdx]!.parts![partIdx]!
}

function mockGeminiRenderContext(events: Event[]) {
  return {
    events,
    tools: [],
    session: {} as never,
...
describe('Gemini serialization', () => {
  describe('serializeContext', () => {
    it('should extract system events as systemInstruction', () => {
      ...
      const result = serializeContext(mockGeminiRenderContext(events))
    ...
    it('should serialize thought events without thoughtSignature as text', () => {
      const events: Event[] = [createEvent<ThoughtEvent>({ type: 'thought', text: 'No signature' })]
    ...
    it('should add synthetic thoughtSignature to tool_call without providerContext', () => {
      const callId = createCallId()
      const events: Event[] = [
        createEvent<ToolCallEvent>({
          type: 'tool_call',
      ...
      const result = serializeContext(mockGeminiRenderContext(events))
      const part = getPart(result.contents, 0, 0)

      expect(part).toMatchObject({
        functionCall: { name: 'lookup', args: { key: 'test' } },
    ...
    it('should serialize tool_result with base64 images as inline data parts', () => {
      const callId = createCallId()
      const events: Event[] = [
        createEvent<ToolResultEvent>({
          type: 'tool_result',
      ...
      expect(getPart(result.contents, 0, 0)).toMatchObject({
        functionResponse: {
          name: 'take_screenshot',
          response: { success: true, width: 800, height: 600 },
        },
      })
      expect(getPart(result.contents, 0, 1)).toMatchObject({
        text: '[Image from tool result:]',
      })
      expect(getPart(result.contents, 0, 2)).toMatchObject({
        inlineData: {
          mimeType: 'image/png',
          data: 'iVBORw0KGgoAAAANSUhEUg==',
        },
    ...
    it('should serialize tool_result with URL images as fileData parts', () => {
      const callId = createCallId()
      const events: Event[] = [
        createEvent<ToolResultEvent>({
          type: 'tool_result',
    ...
    it('should serialize tool_result with multiple images', () => {
      ...
      expect(result.contents[0].parts).toHaveLength(5)
      expect(getPart(result.contents, 0, 0)).toHaveProperty('functionResponse')
      expect(getPart(result.contents, 0, 1)).toMatchObject({
        text: '[Image from tool result:]',
      })
      expect(getPart(result.contents, 0, 2)).toMatchObject({
      ...
      expect(getPart(result.contents, 0, 3)).toMatchObject({
        text: '[Image from tool result:]',
      })
      expect(getPart(result.contents, 0, 4)).toMatchObject({
        inlineData: { mimeType: 'image/jpeg', data: 'image2data==' },
      })
    })
  ...
  describe('parseResponse', () => {
    ...
    it('should parse text part as assistant event', () => {
      ...
      const result = parseResponse(parts, undefined, undefined, testInvocationId, 'test_agent')
    ...
    it('should parse thought part as thought event', () => {
      const parts: Part[] = [{ thought: true, text: 'Thinking...', thoughtSignature: 'sig_123' }]
    ...
    it('should handle empty args in functionCall', () => {
      const parts: Part[] = [{ functionCall: { name: 'get_time', args: undefined } }]
    ...
    it('should aggregate multiple thought chunks into single event', () => {
      ...
      const thoughtEvents = result.stepEvents.filter((e) => e.type === 'thought')
    ...
    it('should aggregate multiple assistant chunks into single event', () => {
      ...
      const assistantEvents = result.stepEvents.filter((e) => e.type === 'assistant')
      expect(assistantEvents).toHaveLength(1)
      expect(assistantEvents[0].type === 'assistant' && assistantEvents[0].text).toBe(
    ...
    it('should handle interleaved chunks with function calls', () => {
      ...
      expect(result.stepEvents).toHaveLength(2)
      expect(result.stepEvents[0].type).toBe('thought')
      expect(result.stepEvents[0].type === 'thought' && result.stepEvents[0].text).toBe(
        'Let me think... I should call a function.',
      )
      expect(result.stepEvents[1].type).toBe('tool_call')
    })
    ...
    it('should handle empty parts array', () => {
      const result = parseResponse([], undefined, undefined, testInvocationId, 'test_agent')
    ...
    it('should not propagate signature to second parallel FC', () => {
      ...
      expect(result.toolCalls[1].providerContext?.data).not.toHaveProperty('thoughtSignature')
    })

    it('should filter empty text parts', () => {
      const parts: Part[] = [{ text: '' }, { thought: true, text: '' }, { text: 'Actual content' }]

      const result = parseResponse(parts, undefined, undefined, testInvocationId, 'test_agent')

      expect(result.stepEvents).toHaveLength(1)
      expect(result.stepEvents[0].type).toBe('assistant')
      expect(result.stepEvents[0].type === 'assistant' && result.stepEvents[0].text).toBe(
        'Actual content',
      )
    })
  })
