...
function getDef(schema: z.ZodType): ZodDef {
  return { ...schema.def, description: schema.description } as ZodDef
}
...
function findReused(schema: z.ZodType): Set<Identity> {
  ...
  function visit(s: z.ZodType): void {
    const d = getDef(s)
    const isObject = d.type === 'object'
    const isLazy = d.type === 'lazy'
...
function getTypeName(schema: z.ZodType, ctx: RenderContext): string {
  ...
  if (t === 'literal') return (d.values ?? []).map((v) => JSON.stringify(v)).join(' | ')
  if (t === 'enum')
  ...
  if (t === 'array' && d.element) {
    ...
    return `${getTypeName(d.element, ctx)}[]`
  }
  if ((t === 'object' || t === 'lazy') && ctx.refs.has(id)) {
