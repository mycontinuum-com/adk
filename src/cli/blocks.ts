...
export type DisplayEvent = Event | DeltaBatchEvent | ContextChildItem | PendingBracket
...
function collapseDeltaEvents(events: CLIEvent[], idPrefix: string): DisplayEvent[] {
...
function collapseBlockDeltas(block: InvocationBlock): void {
  block.events = collapseDeltaEvents(block.events as CLIEvent[], block.invocationId)
  ...
  for (const contextBlock of block.contextBlocks) {
    ...
    for (const event of events) {
      if (event.type === 'delta_batch') {
        ...
        const assistantDelta = deltaBatches.find((d) => d.deltaType === 'assistant_delta')
        if (assistantDelta) {
        ...
        const thoughtDelta = deltaBatches.find((d) => d.deltaType === 'thought_delta')
        if (thoughtDelta) {
          streamingMap.set((event as { id: string }).id, {
...
export function getEventsInDisplayOrder(
  ...
  function outputContextBlocks(contextBlocks: ContextBlock[]): void {
    for (const ctx of contextBlocks) {
      ...
      const hasAssistantEvent = ctx.producedEvents.some((e) => e.type === 'assistant')
      const hasThoughtEvent = ctx.producedEvents.some((e) => e.type === 'thought')
  ...
  function traverse(block: InvocationBlock): void {
    ...
    if (block.kind === 'loop' && block.childMap) {
      for (const event of block.events) {
        if (event.type === 'invocation_start') {
          const childBlock = block.childMap.get((event as InvocationStartEvent).invocationId)
          if (childBlock) {
            traverse(childBlock)
          }
        } else if (!INVOCATION_EVENT_TYPES.has(event.type)) {
...
export function getPendingBrackets(blocks: InvocationBlock[]): PendingBracket[] {
