...
export function YieldHandler({ yieldedTools, onResume }: YieldHandlerProps): React.ReactElement {
  ...
  const currentCall = yieldedTools[currentIndex]

  const argsText = useMemo(() => formatArgs(currentCall.args), [currentCall.args])
  const wrappedArgsLines = useMemo(() => wrapText(argsText, contentWidth), [argsText, contentWidth])
  ...
  const handleSubmit = (value: string) => {
    ...
    if (currentIndex < yieldedTools.length - 1) {
  ...
  const argsScrollPercent =
    maxArgsOffset > 0 ? Math.round((effectiveArgsOffset / maxArgsOffset) * 100) : 0
  ...
  return (
    ...
    >
      <Text bold color="yellowBright">
        ⧗ Yield - Awaiting Input ({currentIndex + 1}/{yieldedTools.length})
      </Text>
      ...
      <Box marginTop={1} flexDirection="column">
        ...
        {currentCall.args != null &&
          (typeof currentCall.args !== 'object' || Object.keys(currentCall.args).length > 0) && (
