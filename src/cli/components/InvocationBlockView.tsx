...
export function InvocationBlockView({
  ...
  return (
    <Box flexDirection="column">
      <Text>
        <Text color="cyanBright" dimColor={false}>
          ┌─
        </Text>
        ...
        {block.handoffOrigin?.type === 'spawn' && (
          <Text color="gray" dimColor>
            :spawn
          </Text>
        )}
        {block.handoffOrigin?.type === 'dispatch' && (
          <Text color="gray" dimColor>
            :dispatch
          </Text>
        )}
        {block.handoffOrigin?.type === 'transfer' && (
          <Text color="gray" dimColor>
            :transfer
          </Text>
        )}
      ...
      <Box flexDirection="column" marginLeft={1} paddingLeft={1}>
        {block.events.map((event, idx) => {
          ...
          return <EventLine key={`${event.type}-${idx}`} event={event} isSelected={isSelected} />
        })}
      ...
      <Text>
        ...
        {block.state === 'yielded' ? (
          <>
            ...
            {block.yieldedToolIds?.length && (
              <Text dimColor> awaiting {block.yieldedToolIds.join(', ')}</Text>
            )}
          </>
