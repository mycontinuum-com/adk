...
describe('findBlockEndEventIndex', () => {
  ...
  it('finds context_end from context_start', () => {
    ...
    expect(findBlockEndEventIndex(lines, contextStartIdx)).toBe(contextEndEventIndex)
  })

  it('finds the next sibling block end when on a block_end', () => {
    const block1 = createMockBlock('inv-1', 'Agent1')
    block1.events = [createMockStartEvent('inv-1'), createMockEndEvent('inv-1')]

    const block2 = createMockBlock('inv-2', 'Agent2')
    block2.events = [createMockStartEvent('inv-2'), createMockEndEvent('inv-2')]
  ...
  it('finds block_end from an event inside the block', () => {
    ...
    const eventLineIdx = lines.findIndex(
      (l) => l.type === 'event' && (l.event as { id?: string })?.id === 'assistant-1',
    )
    const contextEndIdx = lines.findIndex((l) => l.type === 'context_end')

    expect(findBlockEndEventIndex(lines, eventLineIdx)).toBe(lines[contextEndIdx].eventIndex)
  })
})
...
describe('findBlockStartEventIndex', () => {
  ...
  it('finds context_start from context_end', () => {
    ...
    expect(findBlockStartEventIndex(lines, contextEndIdx)).toBe(contextStartEventIndex)
  })
  ...
  it('finds context_start from an event inside the context', () => {
    ...
    expect(findBlockStartEventIndex(lines, eventLineIdx)).toBe(lines[contextStartIdx].eventIndex)
  })
})

describe('getLineIndexForEvent', () => {
  it('returns 0 when event is not found', () => {
    const lines: FlattenedLine[] = [{ key: 'test', type: 'event', depth: 0, eventIndex: 5 }]
