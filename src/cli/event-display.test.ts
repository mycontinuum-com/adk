...
function createThoughtEvent(text: string): ThoughtEvent {
  return {
    id: 'thought-1',
    type: 'thought',
    createdAt: Date.now(),
...
function createAssistantEvent(text: string): AssistantEvent {
  return {
    id: 'assistant-1',
    type: 'assistant',
    createdAt: Date.now(),
...
describe('extractCurrentThoughtBlock', () => {
  ...
  it('returns text from the last thought block header', () => {
    ...
    expect(extractCurrentThoughtBlock(text)).toBe('**Second Block**\n\nContent of second block.')
  })
