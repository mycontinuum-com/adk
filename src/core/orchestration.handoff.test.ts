...
import type { InvocationStartEvent, InvocationEndEvent, ToolResultEvent } from '../types'
...
const createMathExpert = () =>
  agent({
    ...
    context: [injectSystemMessage('Math expert.'), includeHistory({ scope: 'invocation' })],
  })
...
const createResearchAgent = () =>
  agent({
    name: 'research_agent',
    model: openai('gpt-4o-mini'),
    context: [injectSystemMessage('Research agent.'), includeHistory({ scope: 'invocation' })],
  })

const createRunTool = (targetAgent: ReturnType<typeof createMathExpert>) =>
  app.tool({
    name: targetAgent.name,
    description: `Call ${targetAgent.name}`,
    schema: z.object({ task: z.string() }),
    execute: async (ctx) => {
      const result = await ctx.run(targetAgent, ctx.args.task)
      return {
        agent: targetAgent.name,
        status: result.status,
        output: result.output.value ?? result.output.text,
      }
    },
  })
...
const createSpawnTool = (targetAgent: ReturnType<typeof createResearchAgent>) =>
  app.tool({
    ...
    execute: (ctx) => {
      const handle = ctx.spawn(targetAgent, ctx.args.topic)
      return {
        agent: targetAgent.name,
...
const createDispatchTool = (targetAgent: ReturnType<typeof createResearchAgent>) =>
  app.tool({
    name: `dispatch_${targetAgent.name}`,
    description: `Dispatch ${targetAgent.name}`,
    schema: z.object({ topic: z.string() }),
    execute: (ctx) => {
      const handle = ctx.dispatch(targetAgent, ctx.args.topic)
      return {
        agent: targetAgent.name,
        invocationId: handle.invocationId,
        status: 'dispatched',
...
describe('call pattern', () => {
  test('calls synchronously and emits proper events', async () => {
    ...
    const coordinator = testAgent({
      name: 'coordinator',
      tools: [createRunTool(mathExpert)],
    })
    ...
    const ends = [...events].filter((e) => e.type === 'invocation_end') as InvocationEndEvent[]
    const callStart = starts.find(
      (e) => e.agentName === 'math_expert' && e.handoffOrigin?.type === 'run',
    )
    const callEnd = ends.find((e) => e.agentName === 'math_expert')

    expect(callStart?.handoffOrigin?.type).toBe('run')
    ...
    expect(toolResults.find((r) => r.name === 'math_expert')?.result).toMatchObject({
      agent: 'math_expert',
      status: 'completed',
    })
  })
  ...
  test('propagates yields from called agent and resumes correctly', async () => {
    ...
    const callYielding = app.tool({
      name: 'yielding_expert',
      description: 'Call yielding expert',
      schema: z.object({ task: z.string() }),
      execute: async (ctx) => {
        const result = await ctx.run(yieldingExpert, ctx.args.task)
    ...
    const { status } = await runTest(coordWithYielding, [
      ...
      model({
        toolCalls: [{ name: 'yielding_expert', args: { task: 'Confirm this' } }],
      }),
      model({
...
describe('transfer pattern', () => {
  test('transfers control and emits proper events', async () => {
    ...
    const toolResults = [...events].filter((e) => e.type === 'tool_result') as ToolResultEvent[]
    expect(toolResults.find((r) => r.name === 'transfer_to_specialist')?.result).toMatchObject({
      transfer: true,
      agent: 'specialist',
    })
  })
  ...
  test('passes transfer context via state', async () => {
    ...
    expect(session.state.transferContext).toBe('Rare genetic conditions')
  })
})
...
describe('spawn pattern', () => {
  test('spawns and returns immediately with proper events', async () => {
    ...
    const spawnStart = starts.find(
      (e) => e.agentName === 'research_agent' && e.handoffOrigin?.type === 'spawn',
    )

    expect(spawnStart?.handoffOrigin?.type).toBe('spawn')

    const toolResults = [...events].filter((e) => e.type === 'tool_result') as ToolResultEvent[]
    expect(toolResults.find((r) => r.name === 'spawn_research_agent')?.result).toMatchObject({
...
describe('dispatch pattern', () => {
  test('dispatches fire-and-forget with proper events', async () => {
    ...
    const dispatchStart = starts.find(
      (e) => e.agentName === 'research_agent' && e.handoffOrigin?.type === 'dispatch',
    )

    expect(dispatchStart?.handoffOrigin?.type).toBe('dispatch')

    const toolResults = [...events].filter((e) => e.type === 'tool_result') as ToolResultEvent[]
    expect(toolResults.find((r) => r.name === 'dispatch_research_agent')?.result).toMatchObject({
      agent: 'research_agent',
      status: 'dispatched',
    })
  })
...
describe('multiple orchestrations', () => {
  test('coordinator can use multiple call tools sequentially', async () => {
    ...
    const coordinator = testAgent({
      tools: [createRunTool(mathAgent), createRunTool(writingAgent)],
    })
