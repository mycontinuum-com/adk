...
import { testAgent, createTestSession, setupAdkMatchers, MockAdapter } from '../testing'
...
describe('streaming and abort', () => {
  ...
  describe('delta streaming', () => {
    test('streams delta events when streamChunks is enabled', async () => {
      mockAdapter.setResponses([{ text: 'Hello World!', streamChunks: true, chunkSize: 5 }])
    ...
    test('partial stream consumption yields only consumed events', async () => {
      mockAdapter.setResponses([{ text: 'Hello World!', streamChunks: true, chunkSize: 3 }])
    ...
    test('consuming all events yields invocation and streaming events', async () => {
      mockAdapter.setResponses([{ text: 'Complete', streamChunks: true, chunkSize: 4 }])
  ...
  describe('hooks onEvent callback', () => {
  ...
  describe('timeout', () => {
    ...
    test('timeout during streaming produces timeout error', async () => {
      ...
      try {
        await runner.run(testAgent(), createTestSession('Test'), { timeout: 50 })
  ...
  describe('stream consumption', () => {
    test('throws if stream consumed twice', async () => {
      ...
      expect(() => stream[Symbol.asyncIterator]()).toThrow('Stream already consumed')
    })
  })
})
