...
import type { StreamEvent, FunctionTool } from '../types'
...
describe('Event Consistency', () => {
  ...
  const collectEvents = async (
    ...
    options?: { tools?: FunctionTool[] },
  ) => {
    mockAdapter.setResponses(responses)
    const events: StreamEvent[] = []
    const session = createTestSession('Test')

    for await (const event of runner.run(testAgent({ tools: options?.tools }), session)) {
      events.push(event)
    }
  ...
  describe('1. Uniqueness - each event emitted exactly once', () => {
    test('thought events: exactly one per reasoning segment', async () => {
      const { events } = await collectEvents([{ thought: 'Thinking...', text: 'Response' }])
    ...
    test('tool_call events: each call appears exactly once', async () => {
      ...
      const { events } = await collectEvents(
        [{ toolCalls: [{ name: 'test_tool', args: { x: 1 } }] }, { text: 'Done' }],
        { tools: [myTool] },
      )
    ...
    test('tool_call events: multiple calls each appear once', async () => {
      const toolA = app.tool({
        name: 'tool_a',
        description: 'A',
        schema: z.object({}),
        execute: () => ({}),
      })
      const toolB = app.tool({
        name: 'tool_b',
        description: 'B',
        schema: z.object({}),
        execute: () => ({}),
      ...
      const names = toolCallEvents.map((e) => (e.type === 'tool_call' ? e.name : ''))
  ...
  describe('2. Completeness - events contain full, unfragmented content', () => {
    test('thought event has complete text', async () => {
      const { events } = await collectEvents([{ thought: 'First. Second. Third.', text: 'Done' }])

      const thought = events.find((e) => e.type === 'thought')
      expect(thought?.type === 'thought' && thought.text).toBe('First. Second. Third.')
    })

    test('assistant event has complete text', async () => {
      const { events } = await collectEvents([{ text: 'Hello, this is a complete response.' }])
    ...
    test('streaming produces complete event with full accumulated text', async () => {
      ...
      const assistant = events.find((e) => e.type === 'assistant')
      expect(assistant?.type === 'assistant' && assistant.text).toBe('Streamed response content')
    })
    ...
    test('all events have invocationId', async () => {
      ...
      const { events } = await collectEvents(
        [{ thought: 'Thinking', toolCalls: [{ name: 'test', args: {} }] }, { text: 'Done' }],
        { tools: [myTool] },
      )
  ...
  describe('3. Ordering - events arrive in correct sequence', () => {
    ...
    test('deltas arrive before complete event', async () => {
      const { events } = await collectEvents([{ text: 'Response', streamChunks: true }])
  ...
  describe('4. Parity - iterator and callbacks receive identical events', () => {
    test('iterator and onStream receive same events', async () => {
      ...
      for await (const event of runner.run(testAgent(), createTestSession('Test'), {
        hooks: [{ onEvent: (e) => callbackEvents.push(e) }],
      })) {
    ...
    test('event IDs match between iterator and hooks', async () => {
    ...
    test('event types match between iterator and hooks', async () => {
      mockAdapter.setResponses([{ thought: 'Thinking', text: 'Response', streamChunks: true }])
  ...
  describe('5. Persistence - session state is correct', () => {
    test('session contains complete events (thought, assistant)', async () => {
      const { session } = await collectEvents([{ thought: 'Thinking', text: 'Response' }])

      const sessionThought = session.events.find((e) => e.type === 'thought')
      const sessionAssistant = session.events.find((e) => e.type === 'assistant')
    ...
    test('session contains tool events', async () => {
      ...
      const sessionToolCall = session.events.find((e) => e.type === 'tool_call')
      const sessionToolResult = session.events.find((e) => e.type === 'tool_result')
    ...
    test('session does not contain delta events', async () => {
      const { session } = await collectEvents([{ text: 'Response', streamChunks: true }])
    ...
    test('session event IDs match yielded event IDs', async () => {
      const { events, session } = await collectEvents([{ thought: 'Thinking', text: 'Response' }])
  ...
  describe('6. Multi-turn consistency', () => {
    test('events from first turn are available in second turn context', async () => {
      ...
      session.input.message('Second message')
