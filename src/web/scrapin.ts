import type { FetchPipeline, FetchPageOptions, FetchPageResult, PipelineCache } from './types'

const LINKEDIN_PERSON_PATTERN = /linkedin\.com\/in\/([^/?]+)/
const LINKEDIN_COMPANY_PATTERN = /linkedin\.com\/company\/([^/?]+)/

export interface ScrapinPersonResponse {
  success: boolean
  credits_left?: number
  person?: {
...
export interface ScrapinCompanyResponse {
  success: boolean
  company?: {
    linkedInId: string
    name: string
...
function personToMarkdown(data: ScrapinPersonResponse): string {
  ...
  if (person.location) {
    const loc = [person.location.city, person.location.state, person.location.country]
      .filter(Boolean)
      .join(', ')
...
function companyToMarkdown(data: ScrapinCompanyResponse): string {
  ...
  if (company.headquarter) {
    const loc = [company.headquarter.city, company.headquarter.country].filter(Boolean).join(', ')
    if (loc) lines.push(`**Location:** ${loc}`)
  }
...
Or pass directly to the pipeline config.`,
  ...
  async fetchPerson(linkedInUrl: string): Promise<ScrapinPersonResponse> {
    const response = await fetch(`${this.baseUrl}/v1/enrichment/profile?apikey=${this.apiKey}`, {
      method: 'POST',
      headers: {
...
export interface LinkedInPipelineOptions {
  apiKey?: string
  cache?: PipelineCache<FetchPageResult>
}

export function linkedInPipeline(options?: LinkedInPipelineOptions): FetchPipeline {
  const provider = new ScrapinProvider(options?.apiKey)
  ...
  return {
    ...
    async fetch(url: string, _options?: FetchPageOptions): Promise<FetchPageResult> {
      if (cache) {
      ...
      try {
        ...
        if (isPerson) {
          ...
          const title = `${data.person.firstName} ${data.person.lastName} - ${
            data.person.headline || 'LinkedIn'
          }`
          result = {
            success: true,
            url,
