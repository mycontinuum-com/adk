...
describe('URL pattern matching', () => {
  const mockScrapinPipeline = {
    ...
    fetch: vi.fn<(...args: unknown[]) => unknown>().mockResolvedValue({
      success: true,
...
describe('Scrapin provider', () => {
  test('fetchPerson returns structured data', async () => {
    ...
    const mockFetch = vi.fn<(...args: unknown[]) => unknown>().mockResolvedValue({
      ok: true,
    ...
    try {
      ...
      const result = await provider.fetchPerson('https://linkedin.com/in/johndoe')
  ...
  test('fetchCompany returns structured data', async () => {
    ...
    try {
      const provider = new ScrapinProvider('test-api-key')
      const result = await provider.fetchCompany('https://linkedin.com/company/acme')
...
describe('Scrapin pipeline markdown output', () => {
  test('pipeline returns markdown content for person', async () => {
    ...
    try {
      const pipeline = linkedInPipeline({ apiKey: 'test-key' })
...
describe('Tool configuration', () => {
  ...
  test('webSearch includes country when allowCountryOverride is true', () => {
    ...
    expect(tool.schema.safeParse({ query: 'test', country: 'US' }).success).toBe(true)
  })
  ...
  test('fetchPage creates tool with correct schema', () => {
    ...
    expect(tool.description).toContain('Fetch content from')
    expect(tool.schema.safeParse({ urls: 'https://example.com' }).success).toBe(true)
    expect(tool.schema.safeParse({ urls: ['https://a.com', 'https://b.com'] }).success).toBe(true)
    expect(
...
describe('Provider initialization', () => {
  test('SerperProvider throws without API key', () => {
    ...
    try {
      expect(() => new SerperProvider()).toThrow('No Serper API key configured')
    } finally {
      if (originalEnv) process.env.SERPER_API_KEY = originalEnv
    }
  })

  // A module that merely defines a research agent must import without the key: constructing the
  // default provider while BUILDING the tool made the agent unloadable rather than unrunnable.
  test('webSearch builds without SERPER_API_KEY and only demands it on execute', async () => {
    const originalEnv = process.env.SERPER_API_KEY
    delete process.env.SERPER_API_KEY

    try {
      const app = adk()
      const tool = app.use(webSearch())

      expect(tool.name).toBe('web_search')

      await expect(tool.execute?.({ args: { query: 'test' } } as never)).rejects.toThrow(
  ...
  test('ScrapinProvider throws without API key', () => {
    ...
    try {
      expect(() => new ScrapinProvider()).toThrow('No Scrapin API key configured')
    } finally {
  ...
  test('linkedInPipeline creates pipeline with correct patterns', () => {
    ...
    expect(pipeline.patterns[0].test('https://linkedin.com/in/user')).toBe(true)
    expect(pipeline.patterns[1].test('https://linkedin.com/company/corp')).toBe(true)
  })
})
