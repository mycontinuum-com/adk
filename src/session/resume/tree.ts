...
export const endReasonToState: Record<InvocationEndReason, InvocationState> = {
  completed: 'completed',
  error: 'error',
  aborted: 'aborted',
  max_steps: 'completed',
    ...
    switch (event.type) {
      ...
      case 'invocation_yield': {
        ...
        if (node) {
          ...
          node.yieldedToolIds = event.yieldedToolIds
        }
        break
      }
      case 'invocation_resume': {
        ...
        if (node) {
          node.state = 'running'
          node.yieldIndex = event.yieldIndex
          node.yieldedToolIds = undefined
        }
        break
      }
      case 'tool_call': {
        if (event.invocationId) {
          nodeMap.get(event.invocationId)?.toolCalls.set(event.callId, { call: event })
        }
        break
      }
      case 'tool_yield': {
...
function collectYieldedNodes(node: InvocationNode): InvocationNode[] {
  return node.state === 'yielded'
    ? [node, ...node.children.flatMap(collectYieldedNodes)]
    : node.children.flatMap(collectYieldedNodes)
}

export function findYieldedNodes(nodes: InvocationNode[]): InvocationNode[] {
  return nodes.flatMap(collectYieldedNodes)
}

export function getUnresolvedYields(node: InvocationNode): UnresolvedYield[] {
  const unresolved: UnresolvedYield[] = []

  if (node.state === 'yielded' && node.yieldedToolIds) {
    for (const callId of node.yieldedToolIds) {
...
export function hasUnresolvedYields(node: InvocationNode): boolean {
  if (node.state === 'yielded' && node.yieldedToolIds) {
    const hasUnresolved = node.yieldedToolIds.some((callId) => {
      const toolEntry = node.toolCalls.get(callId)
...
export function getNodePath(nodes: InvocationNode[], invocationId: string): InvocationNode[] {
  for (const root of nodes) {
