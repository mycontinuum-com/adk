...
export function validateResumeState(events: readonly Event[]): UnresolvedYield[] {
...
export function assertReadyToResume(events: readonly Event[]): void {
  ...
  if (unresolvedYields.length > 0) {
    const details = unresolvedYields
      ...
      .join(', ')
    throw new Error(`Cannot resume: missing tool results for pending yields: ${details}`)
  }
}
...
function buildContextForPath(
  ...
  switch (runnable.kind) {
    case 'agent':
      return baseContext
    case 'sequence':
      return path.length === 1 ? baseContext : buildSequenceContext(path, runnable)
    case 'parallel':
      return path.length === 1 ? baseContext : buildParallelContext(path, runnable)
    case 'loop':
...
function buildSequenceContext(
  ...
  let stepIndex = runnable.runnables.findIndex((step) => step.name === childInPath.agentName)
  if (stepIndex === -1) {
    stepIndex = root.children.findIndex((c) => c.invocationId === childInPath.invocationId)
...
function buildParallelContext(
  ...
  const findBranchIndex = (child: InvocationNode): number => {
    ...
    return childIdx >= 0 && childIdx < runnable.runnables.length ? childIdx : -1
  }
