...
import { computeResumeContext, validateResumeState, assertReadyToResume } from './context'
...
const start = (id: string, name: string, parent?: string): InvocationStartEvent => ({
  id: `${id}-start`,
...
const yld = (
  ...
  invocationId: id,
  agentName: name,
  yieldedToolIds: calls,
  yieldIndex: idx,
  parentInvocationId: parent,
  awaitingInput,
})

const resume = (id: string, name: string, idx: number): InvocationResumeEvent => ({
  id: `${id}-resume`,
...
describe('computeResumeContext', () => {
  ...
  test('returns undefined when tool results missing', () => {
    const events: Event[] = [start('i', 'a'), call('c', 'i'), yld('i', 'a', ['c'], 0)]
    expect(computeResumeContext(events, agent('a'))).toBeUndefined()
  })

  test('simple agent yield with resolved result', () => {
    const events: Event[] = [start('i', 'a'), call('c', 'i'), yld('i', 'a', ['c'], 0), input('c')]
  ...
  test('deeply nested sequences (4 levels)', () => {
    ...
    for (const id of ids) {
      ...
      expect(ctx.stepIndex).toBe(0)
      if (ctx.stepResumeContext) ctx = ctx.stepResumeContext as SequenceResumeContext
    }
  })
  ...
  test('multi-turn: turn 1 completed, turn 2 yielded with resolved input', () => {
    const a = agent('a')
    const events: Event[] = [
      // Turn 1: fully completed root invocation
      start('t1', 'a'),
      end('t1', 'a', 'completed'),
      // Turn 2: new root invocation that yielded
      start('t2', 'a'),
      call('c', 't2'),
      yld('t2', 'a', ['c'], 0),
      input('c'),
    ...
    expect(ctx).toMatchObject({ invocationId: 't2', yieldIndex: 0 })
  })

  test('single turn completed with no yields returns undefined', () => {
    const a = agent('a')
    const events: Event[] = [start('i', 'a'), end('i', 'a', 'completed')]

    expect(computeResumeContext(events, a)).toBeUndefined()
  })
})
...
describe('validateResumeState', () => {
  ...
  test('returns unresolved yields', () => {
    const events: Event[] = [start('i', 'a'), call('c', 'i'), yld('i', 'a', ['c'], 0)]
    expect(validateResumeState(events)).toEqual([{ agentName: 'a', callId: 'c' }])
  })
})
