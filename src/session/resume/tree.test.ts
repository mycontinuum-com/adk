...
describe('buildInvocationTree', () => {
  ...
  test('maps all end reasons correctly', () => {
    const directReasons: InvocationEndEvent['reason'][] = [
      'completed',
      'error',
      'aborted',
      'transferred',
  ...
  test('maps limit end reasons to completed', () => {
    const limitReasons: InvocationEndEvent['reason'][] = [
      'max_steps',
      'max_turns',
      'max_duration',
      'inactivity_timeout',
  ...
  test('handles invocation_yield event', () => {
    ...
    expect(tree[0].yieldedToolIds).toEqual(['call-1', 'call-2'])
  })

  test('handles invocation_resume event and clears yieldedToolIds', () => {
    const events: Event[] = [
      createInvocationStart('inv-1', 'agent1'),
    ...
    expect(tree[0].state).toBe('running')
    expect(tree[0].yieldIndex).toBe(0)
    expect(tree[0].yieldedToolIds).toBeUndefined()
  })
  ...
  test('ignores tool events without matching invocation', () => {
    const events: Event[] = [
      ...
      createToolResult('orphan-call', 'orphanTool', 'inv-nonexistent', 'ignored'),
    ]
    const tree = buildInvocationTree(events)
  ...
  test('throws InvocationTreeError when child references non-existent parent', () => {
    const events: Event[] = [createInvocationStart('inv-child', 'childAgent', 'inv-missing-parent')]
...
describe('findNode', () => {
  test('returns undefined for empty tree or non-existent node', () => {
    ...
    const tree = buildInvocationTree([createInvocationStart('inv-1', 'agent1')])
...
describe('getNodePath', () => {
  ...
  test('returns correct path to nodes at any depth', () => {
    ...
    expect(grandchildPath.map((n) => n.agentName)).toEqual(['parent', 'child2', 'grandchild'])
  })
})

function createYieldedTestNode(
  invocationId: string,
  yieldedToolIds: string[],
...
function createRunningTestNode(
  invocationId: string,
  children: InvocationNode[] = [],
): InvocationNode {
  return {
...
describe('hasUnresolvedYields', () => {
  ...
  test('returns false for non-yielded nodes or yielded with no pending calls', () => {
    expect(hasUnresolvedYields(createRunningTestNode('inv-1'))).toBe(false)

    const yieldedNoPending = createYieldedTestNode('inv-1', [], new Map())
    yieldedNoPending.yieldedToolIds = undefined
  ...
  test('returns true when yields are unresolved, false when resolved', () => {
    ...
    const nodeWithUnresolved = createYieldedTestNode('inv-1', ['call-1', 'call-2'], toolCalls)
    expect(hasUnresolvedYields(nodeWithUnresolved)).toBe(true)

    toolCalls.set('call-1', createToolCallEntry('call-1', true))
    const nodeAllResolved = createYieldedTestNode('inv-2', ['call-1', 'call-2'], toolCalls)
    ...
    const childWithUnresolved = createYieldedTestNode('inv-child', ['call-child'], childToolCalls)
    const compositionNode = createYieldedTestNode('inv-composition', ['call-child'], new Map(), [
      childWithUnresolved,
    ])
  ...
  test('checks children recursively', () => {
    ...
    const grandchild = createYieldedTestNode('inv-grandchild', ['call-gc'], grandchildToolCalls)
    const child = createRunningTestNode('inv-child', [grandchild])
    const parent = createRunningTestNode('inv-parent', [child])

    expect(hasUnresolvedYields(parent)).toBe(true)

    grandchildToolCalls.set('call-gc', createToolCallEntry('call-gc', true))
    const resolvedGrandchild = createYieldedTestNode(
      'inv-grandchild',
      ['call-gc'],
      grandchildToolCalls,
    )
    const resolvedChild = createRunningTestNode('inv-child', [resolvedGrandchild])
    const resolvedParent = createRunningTestNode('inv-parent', [resolvedChild])
...
describe('getUnresolvedYields', () => {
  ...
  test('returns unresolved yields from current node and children', () => {
    ...
    parentToolCalls.set('call-parent', createToolCallEntry('call-parent', false))

    const childToolCalls = new Map<string, ToolEntry>()
    childToolCalls.set('call-child', createToolCallEntry('call-child', false))
    childToolCalls.set('call-resolved', createToolCallEntry('call-resolved', true))

    const child: InvocationNode = {
      invocationId: 'inv-child',
      agentName: 'child-runnable',
      state: 'yielded',
      yieldedToolIds: ['call-child', 'call-resolved'],
      children: [],
...
describe('integration scenarios', () => {
  test('complete yield-resume cycle updates tree correctly', () => {
    ...
    events.push(createToolResult('call-1', 'yielding_tool', 'inv-1', 'approved'))
