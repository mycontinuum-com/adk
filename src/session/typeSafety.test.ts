...
function typeSafetyMockExecute(ctx: StepContext<typeof testSchema>): void {
...
function typeSafetyMockWhile(ctx: LoopContext<typeof testSchema>): boolean {
  const mode: 'triage' | 'consultation' | 'followup' | undefined = ctx.state.mode
...
function typeSafetyMockMerge(ctx: ParallelMergeContext<typeof testSchema>): Event[] {
  const mode: 'triage' | 'consultation' | 'followup' | undefined = ctx.state.mode
...
function typeSafetyThrowRespond(_text: string): never {
  throw new Error('respond')
}

function typeSafetyMockRenderer(
  ctx: RenderContext<typeof testSchema>,
): RenderContext<typeof testSchema> {
  const mode: 'triage' | 'consultation' | 'followup' | undefined = ctx.state.mode
...
describe('TypedState compile-time type safety', () => {
  describe('session scope (shorthand access)', () => {
    ...
    it('allows clearing fields via update()', () => {
      const session = new BaseSession<typeof testSchema>('test-app')
      const state = session.state
      ...
      state.update({ mode: undefined })
      expect(state.mode).toBeUndefined()

      state.count = 42
      state.update({ count: undefined })
  ...
  describe('user scope (explicit access)', () => {
    ...
    it('allows reading any keys on shared scopes', () => {
      const state = {} as TestState
    ...
    it('allows writing any keys on shared scopes', () => {
      const state = {} as TestState
  ...
  describe('patient scope (explicit access)', () => {
    ...
    it('allows any keys on shared scopes', () => {
      const state = {} as TestState
  ...
  describe('undeclared scopes are inaccessible', () => {
    it('rejects access to scopes not in schema', () => {
      const state = {} as TestState

      // @ts-expect-error - 'practice' is not declared in testSchema
      void state.practice

      // @ts-expect-error - 'org' is not declared in testSchema
      void state.org

      // @ts-expect-error - 'team' is not declared in testSchema
      void state.team
  ...
  describe('cross-scope isolation', () => {
    it('session keys are accessible on user scope', () => {
      const state = {} as TestState
...
describe('runtime proxy behavior matches types', () => {
  it('setting and getting values works correctly', () => {
    ...
    session.state.update({ mode: undefined })
    expect(session.state.mode).toBeUndefined()
  })
  ...
  it('update with undefined removes keys', () => {
    const session = new BaseSession<typeof testSchema>('test-app')
...
describe('factory function generics', () => {
  ...
  describe('Step<S> generic', () => {
    it('Step execute receives typed StepContext', () => {
      ...
      const mockExecute: TypedStep['execute'] = typeSafetyMockExecute

      expect(s.kind).toBeUndefined()
      expect(mockExecute).toBeDefined()
    })

    it('StepContext has respond method', () => {
      type RespondSignature = StepContext<typeof testSchema>['respond']

      const respond: RespondSignature = typeSafetyThrowRespond
  ...
  describe('Loop<S> generic', () => {
    it('Loop.while receives typed LoopContext', () => {
      ...
      const mockWhile: TypedLoop['while'] = typeSafetyMockWhile
  ...
  describe('Parallel<S> generic', () => {
    it('Parallel.merge receives typed ParallelMergeContext', () => {
      ...
      const mockMerge: TypedParallel['merge'] = typeSafetyMockMerge
...
describe('OutputConfig type safety', () => {
  ...
  describe('OutputConfig<S, T>', () => {
    ...
    it('OutputSchemaConfig.key is typed to session keys', () => {
      type TypedOutputConfig = OutputConfig<typeof testSchema, number>
      ...
      const _badConfig: TypedOutputConfig = {
        // @ts-expect-error - 'invalid' is not a session key
        key: 'invalid',
        schema: z.number(),
      }
...
describe('FunctionTool<TInput, TOutput, TYield, S> generic', () => {
  it('tool callbacks receive typed ToolContext', () => {
    ...
    const mockExecute: TypedTool['execute'] = typeSafetyMockToolExecute
...
describe('ContextRenderer<S> generic', () => {
  it('renderer receives typed RenderContext', () => {
    ...
    const renderer: TypedRenderer = typeSafetyMockRenderer
...
describe('InvocationContext<S> and ToolContext<S>', () => {
  it('InvocationContext.state type is correctly inferred', () => {
    ...
    // @ts-expect-error - 'invalid' is not a valid mode
    const _invalidMode: ModeType = 'invalid'

    // @ts-expect-error - undefined is not assignable to mode (use update() to clear)
    const _undefinedMode: ModeType = undefined
