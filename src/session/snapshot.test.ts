...
describe('computeStateAtEvent', () => {
  ...
  test('filters by scope', () => {
    const events: Event[] = [
      createStateChangeEvent('1', 'session', 'sessionKey', undefined, 'session'),
      createStateChangeEvent('2', 'user', 'userKey', undefined, 'user'),
      createStateChangeEvent('3', 'patient', 'patientKey', undefined, 'patient'),
    ]
  ...
  test('throws on out of bounds index', () => {
    const events: Event[] = [createStateChangeEvent('1', 'session', 'key', undefined, 'value')]
...
describe('computeAllStatesAtEvent', () => {
  test('computes all scopes in single pass', () => {
    ...
    expect(result.scopedStates.user).toEqual({ userKey: 'u1' })
    expect(result.scopedStates.patient).toEqual({ patientKey: 'p1' })
    expect(result.scopedStates.practice).toEqual({ practiceKey: 'pr1' })
  })
})
...
describe('snapshotAt', () => {
  test('creates full snapshot at event index', () => {
    const events: Event[] = [
      ...
      createStateChangeEvent('2', 'session', 'key', undefined, 'value', 'inv-1'),
  ...
  test('tracks pending yielding calls', () => {
    const events: Event[] = [
      ...
      createToolYieldEvent('3', 'call-1', 'approval', 'inv-1', 'agent_a'),
    ]

    const snapshot = snapshotAt(events, 2)
    expect(snapshot.yieldedTools).toHaveLength(1)
    expect(snapshot.yieldedTools[0].callId).toBe('call-1')
  ...
  test('resolved yielding calls are not pending', () => {
    const events: Event[] = [
      ...
      createToolCallEvent('2', 'call-1', 'approval', 'inv-1', 'agent_a', true),
      createToolYieldEvent('3', 'call-1', 'approval', 'inv-1', 'agent_a'),
      createToolResultEvent('4', 'call-1', 'approval', 'inv-1', 'agent_a'),
    ]

    const snapshot = snapshotAt(events, 3)
    expect(snapshot.yieldedTools).toHaveLength(0)
    expect(snapshot.status).toBe('active')
  })
  ...
  test('builds invocation tree', () => {
    ...
    expect(snapshot.invocationTree[0].children[0].agentName).toBe('child_agent')
  })
  ...
  test('includes all state scopes', () => {
    ...
    expect(snapshot.scopedStates.user).toEqual({ b: 2 })
    expect(snapshot.scopedStates.patient).toEqual({ c: 3 })
    expect(snapshot.scopedStates.practice).toEqual({ d: 4 })
  })
})
...
describe('BaseSession time-travel methods', () => {
  ...
  describe('forkAt', () => {
    test('creates isolated session copy at event index', () => {
      ...
      expect(forked.scopes.user).toBe('user-1')
      expect(forked.scopes.patient).toBe('patient-1')
    ...
    test('forked session can continue execution independently', () => {
      ...
      const state = session.boundState('inv-1')
      session.pushEvent(createInvocationStartEvent('1', 'inv-1', 'agent'))
      session.input.message('First question')
      state.answered = 1

      const forked = session.forkAt(1)

      session.input.message('Continue original')
      forked.input.message('Different path')
