...
import { agent, sequence, parallel, loop, step } from '../agents/factory'
import { adk } from '../api'
...
describe('computePipelineFingerprint', () => {
  ...
  describe('basic fingerprinting', () => {
    ...
    test('different names produce different fingerprints', () => {
      ...
      expect(computePipelineFingerprint(agent1)).not.toBe(computePipelineFingerprint(agent2))
    })

    test('different tools produce different fingerprints', () => {
      const tool1 = app.tool({
        name: 'tool_one',
        description: 'First tool',
        schema: z.object({}),
        execute: () => ({}),
      })
      const tool2 = app.tool({
        name: 'tool_two',
    ...
    test('tool order does not affect fingerprint (tools are sorted)', () => {
      ...
      expect(computePipelineFingerprint(agentAB)).toBe(computePipelineFingerprint(agentBA))
    })
  })
  ...
  describe('sequence fingerprinting', () => {
    test('sequence fingerprint includes child order', () => {
      ...
      expect(computePipelineFingerprint(seqAB)).not.toBe(computePipelineFingerprint(seqBA))
    })
    ...
    test('nested sequences produce distinct fingerprints', () => {
      ...
      expect(computePipelineFingerprint(outerSeq)).not.toBe(computePipelineFingerprint(flatSeq))
    })
  })
  ...
  describe('parallel fingerprinting', () => {
    test('parallel fingerprint includes child order', () => {
      ...
      expect(computePipelineFingerprint(parAB)).not.toBe(computePipelineFingerprint(parBA))
    })
  })
  ...
  describe('loop fingerprinting', () => {
    test('loop fingerprint includes inner runnable', () => {
      ...
      expect(computePipelineFingerprint(loopA)).not.toBe(computePipelineFingerprint(loopB))
    })
  ...
  describe('step fingerprinting', () => {
    test('step fingerprint is based on name', () => {
      ...
      expect(computePipelineFingerprint(step1)).not.toBe(computePipelineFingerprint(step2))
    })
  })
})
