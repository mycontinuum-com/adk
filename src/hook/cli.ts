...
function renderJsonKeys(text: string, keyColor: ColorName, isDimmed: boolean): string {
...
function formatLabel(label: string, width: number, colorName: ColorName): string {
  ...
  const finalizeStreaming = (): boolean => {
    ...
    if (wasThought) {
      if (thoughtBuffer.length > lastRenderedPos) {
        ...
        let rendered = renderThoughtText(remaining, dimThoughts, lastRenderedPos > 0)
      ...
      switch (event.type) {
        case 'thought_delta': {
          ...
          if (safeEnd > lastRenderedPos) {
            ...
            const rendered = renderThoughtText(toRender, dimThoughts, lastRenderedPos > 0)
            process.stdout.write(rendered)
