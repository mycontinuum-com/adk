import type { Event, AssistantEvent, ToolCallEvent, ToolResultEvent } from '../types'
...
describe('ADK Jest Matchers', () => {
  ...
  const createToolCallEvent = (name: string, args: Record<string, unknown>): ToolCallEvent => ({
    id: createEventId(),
    type: 'tool_call',
    createdAt: Date.now(),
    invocationId: TEST_INVOCATION_ID,
  ...
  describe('toHaveToolCall', () => {
    test('passes when tool call exists', () => {
      const events: Event[] = [createToolCallEvent('calculate', { a: 1, b: 2 })]
  ...
  describe('toHaveToolResult', () => {
    test('passes when tool result exists', () => {
      ...
      const events: Event[] = [createToolResultEvent('calculate', { sum: 5 }, callId)]
      expect(events).toHaveToolResult('calculate')
    })

    test('passes when result matches', () => {
      const callId = createCallId()
      const events: Event[] = [createToolResultEvent('add', { sum: 15 }, callId)]
  ...
  describe('toHaveEventSequence', () => {
    ...
    test('filters to only specified types', () => {
      ...
      expect(events).toHaveEventSequence(['user', 'tool_call', 'tool_result', 'assistant'])
    })
  ...
  describe('toHaveStatus', () => {
    ...
    test('works with nested result', () => {
      const obj = { result: { status: 'yielded_tool' } }
      expect(obj).toHaveStatus('yielded_tool')
    })
