...
import { user, setupAdkMatchers } from './index'
  ...
  describe('createTestSession', () => {
    ...
    test('accepts custom options', () => {
      const session = createTestSession('Hi', {
        id: 'custom-id',
        scopes: { user: 'user-123', patient: 'patient-456' },
      })

      expect(session.id).toBe('session_custom-id')
      expect(session.scopes.user).toBe('user-123')
      expect(session.scopes.patient).toBe('patient-456')
    })
  })
