...
describe('runTest() runner', () => {
  ...
  describe('basic execution', () => {
    test('runs agent with simple response', async () => {
      const { status, events } = await runTest(testAgent(), [
        user('Hello'),
        model({ text: 'Hi there!' }),
      ])
  ...
  describe('tool execution', () => {
    test('executes tool calls and continues', async () => {
      const { events, iterations } = await runTest(testAgent({ tools: [addTool] }), [
        user('What is 2 + 3?'),
  ...
  describe('event patterns', () => {
    test('matches event patterns', async () => {
      const { events } = await runTest(testAgent(), [user('Hello'), model({ text: 'Hi!' })])
    ...
    test('matches events with regex in patterns', async () => {
      ...
      const assistantEvents = [...events].filter((e) => e.type === 'assistant')
      expect(assistantEvents.some((e) => 'text' in e && /chicken/.test(e.text as string))).toBe(
        true,
      )
    })
  })
  ...
  describe('iteration tracking', () => {
    test('tracks correct number of iterations', async () => {
      const { iterations } = await runTest(testAgent(), [user('Hello'), model({ text: 'Hi!' })])
