...
import type { Metric, MetricRun, MetricResult, NumberAssertion } from './types'
...
function findFirstEventOfType<T extends Event>(events: Event[], type: T['type']): T | undefined {
  return events.find((e): e is T => e.type === type)
}

function filterEventsByType<T extends Event>(events: Event[], type: T['type']): T[] {
  return events.filter((e): e is T => e.type === type)
}

function computeTotalDuration(events: Event[]): number | undefined {
  const starts = filterEventsByType<InvocationStartEvent>(events, 'invocation_start')
  const ends = filterEventsByType<InvocationEndEvent>(events, 'invocation_end')

  if (starts.length === 0 || ends.length === 0) return undefined

  return ends[ends.length - 1].createdAt - starts[0].createdAt
}

function computeTimeToFirstAssistant(events: Event[]): number | undefined {
  const firstStart = findFirstEventOfType<InvocationStartEvent>(events, 'invocation_start')
  const firstAssistant = findFirstEventOfType<AssistantEvent>(events, 'assistant')

  if (!firstStart || !firstAssistant) return undefined

  return firstAssistant.createdAt - firstStart.createdAt
}

function computeTimeToFirstToolCall(events: Event[]): number | undefined {
  const firstStart = findFirstEventOfType<InvocationStartEvent>(events, 'invocation_start')
  const firstToolCall = findFirstEventOfType<ToolCallEvent>(events, 'tool_call')

  if (!firstStart || !firstToolCall) return undefined

  return firstToolCall.createdAt - firstStart.createdAt
}

function computeModelLatencies(events: Event[]): number[] {
  const latencies: number[] = []
  const modelStarts = filterEventsByType<ModelStartEvent>(events, 'model_start')
  ...
  for (let i = 0; i < Math.min(modelStarts.length, modelEnds.length); i++) {
    ...
    if (end.durationMs !== undefined) {
      ...
      latencies.push(end.createdAt - modelStarts[i].createdAt)
    }
  }
...
function computeToolExecutionTimes(events: Event[]): number[] {
  ...
  const toolResults = filterEventsByType<ToolResultEvent>(events, 'tool_result')
  ...
  for (const call of toolCalls) {
    ...
    if (!result) continue
    times.push(result.createdAt - call.createdAt)
  }

  return times
}

function computeMeasure(events: Event[], measure: TimingMeasure): number | undefined {
  switch (measure) {
    case 'total_duration':
    ...
    case 'model_latency_total': {
      ...
      return latencies.length > 0 ? latencies.reduce((a, b) => a + b, 0) : undefined
    }
