...
interface QueuedItem {
  ...
  result?: ProducerResult
  error?: string
  /** Preserved error class name so consumers can distinguish error types (e.g. OutputParseError). */
  errorName?: string
  abortReason?: string
}
  ...
  private async runProducer<T>(
    ...
    try {
      ...
      this.queue.push({
        ...
        isMain,
        error: err.message,
        errorName: err.name !== 'Error' ? err.name : undefined,
      })
      this.notify()
        ...
        switch (item.type) {
          ...
          case 'error':
            ...
            if (item.isMain) {
              ...
              // Preserve the original error class name so consumers can distinguish error types
              if (item.errorName) {
                thrownError.name = item.errorName
              }
              this.closed = true
            }
          ...
          case 'complete':
            ...
            if (item.isMain && item.result) {
              ...
              if (mainResult.status === 'yielded_tool') {
                this.closed = true
              }
            }
            if (this.checkAllComplete(completedProducers)) {
