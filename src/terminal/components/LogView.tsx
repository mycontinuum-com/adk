  ...
  return (
    <Box flexDirection="column">
      ...
      {visibleLogs.map((log, visibleIdx) => {
        ...
        const maxMsgLen = 200
        const truncatedMsg =
          log.message.length > maxMsgLen ? log.message.slice(0, maxMsgLen) + '...' : log.message
        ...
        return (
          <Box key={log.id}>
            ...
            <Text color={getLevelColor(log.level)} backgroundColor={bg}>
              {getLevelLabel(log.level).padEnd(5)}
            </Text>
            <Text> </Text>
            <Text
              color={log.level === 'error' ? 'red' : log.level === 'warn' ? 'yellow' : undefined}
            >
              {truncatedMsg}
            </Text>
          </Box>
