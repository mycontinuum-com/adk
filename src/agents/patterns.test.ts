...
describe('gated', () => {
  ...
  test('skips wrapped runnable when check signals', async () => {
    let executed = false
    const inner = step({
      name: 'inner',
      execute: () => {
...
describe('cached', () => {
  ...
  test('respects TTL - executes when expired, skips when valid', async () => {
    ...
    expiredSession.input.message('Go')
    expiredSession.state.result = 'old_value'
    const stateEvent = expiredSession.events.find((e) => e.type === 'state_change')
    if (stateEvent) (stateEvent as { createdAt: number }).createdAt = Date.now() - 2000
