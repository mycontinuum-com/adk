...
describe('agent hooks', () => {
  describe('beforeAgent', () => {
    test('beforeAgent can skip execution by returning value', async () => {
      ...
      const { events, iterations, status } = await runTest(agent, [user('Test')])
    ...
    test('beforeAgent returning undefined continues execution', async () => {
      ...
      const { events } = await runTest(agent, [user('Test'), model({ text: 'Normal response' })])
  ...
  describe('afterAgent', () => {
    test('afterAgent can transform output', async () => {
      ...
      const { result } = await runTest(agent, [user('Test'), model({ text: 'Original' })])

      expect(result.status).toBe('completed')
      if (result.status === 'completed') {
        expect(result.output.value).toBe('Original [modified]')
      }
    })
  })
  ...
  describe('beforeModel', () => {
    test('beforeModel can skip model call and return result', async () => {
        ...
        hooks: [
          {
            beforeModel: (_ctx, renderCtx): ModelStepResult | void => {
              const lastUser = renderCtx.events.filter((e) => e.type === 'user').pop()
              if (lastUser?.type === 'user' && lastUser.text.includes('blocked')) {
                return {
                  stepEvents: [
                    {
                      id: 'x',
      ...
      const { events } = await runTest(agent, [user('This is blocked content')])

      expect(events).toHaveAssistantText('Blocked by hook')
    })

    test('synthetic steps from beforeModel do not count against maxSteps', async () => {
      ...
      const agent = testAgent({
        ...
        hooks: [
          {
            beforeModel: (_ctx, renderCtx): ModelStepResult | void => {
              const userEvents = renderCtx.events.filter((e): e is UserEvent => e.type === 'user')
              const lastUser = userEvents[userEvents.length - 1]
              if (lastUser && lastUser.text === 'start') {
                syntheticCalls++
                if (syntheticCalls <= 3) {
                  return {
                    stepEvents: [
                      {
                        id: `synth-${syntheticCalls}`,
                        type: 'assistant',
                        createdAt: Date.now(),
                        invocationId: renderCtx.invocationId,
                        agentName: renderCtx.agentName,
                        text: `Synthetic step ${syntheticCalls}`,
                      },
                    ],
                    toolCalls: [],
                    terminal: syntheticCalls === 3,
                  }
                }
              }
            },
      ...
      const { iterations, status } = await runTest(agent, [user('start')])

      expect(syntheticCalls).toBe(3)
      expect(iterations).toBe(0)
      expect(status).toBe('completed')
    })
  })
  ...
  describe('afterModel', () => {
    test('afterModel can modify result', async () => {
        ...
        hooks: [
          {
            afterModel: (_, result) => ({
              ...
              stepEvents: result.stepEvents.map((e) =>
                e.type === 'assistant' ? { ...e, text: `${e.text} [post-processed]` } : e,
              ),
            }),
          },
        ],
      ...
      const { events } = await runTest(agent, [user('Test'), model({ text: 'Response' })])
  ...
  describe('beforeTool', () => {
    test('beforeTool can block tool execution', async () => {
      ...
      expect(
        toolResults.some((r) => r.type === 'tool_result' && r.error === 'Tool blocked by hook'),
      ).toBe(true)
    })
  })
  ...
  describe('afterTool', () => {
    test('afterTool can modify tool result', async () => {
      ...
      const toolResults = [...events].filter((e) => e.type === 'tool_result')
      const dataResult = toolResults.find((r) => r.type === 'tool_result' && r.name === 'data_tool')
...
describe('tool error handling', () => {
  test('handles tool validation errors', async () => {
    ...
    const toolResults = [...events].filter((e) => e.type === 'tool_result')
    expect(
      toolResults.some((r) => r.type === 'tool_result' && r.error?.includes('Invalid arguments')),
    ).toBe(true)
  })
  ...
  test('handles unknown tool calls', async () => {
    ...
    const toolResults = [...events].filter((e) => e.type === 'tool_result')
    expect(
      toolResults.some((r) => r.type === 'tool_result' && r.error === 'Unknown tool: nonexistent'),
    ).toBe(true)
  })

  test('handles tool execution errors with errorHandler', async () => {
    const failTool = app.tool({
      name: 'fail',
      description: 'Fails',
      schema: z.object({}),
      execute: () => {
    ...
    const toolResults = [...events].filter((e) => e.type === 'tool_result')
    expect(toolResults.some((r) => r.type === 'tool_result' && r.error?.includes('failed'))).toBe(
      true,
    )
...
describe('tool timeout and retry', () => {
  test('tool with timeout times out after configured duration', async () => {
    ...
    const toolResults = [...events].filter((e) => e.type === 'tool_result')
    expect(
      toolResults.some((r) => r.type === 'tool_result' && r.error?.includes('timed out')),
    ).toBe(true)
  })

  test('tool with retry succeeds after failures', async () => {
    let attempts = 0
    const flakyTool = app.tool({
      name: 'flaky',
    ...
    const toolResults = [...events].filter((e) => e.type === 'tool_result')
    const flakyResult = toolResults.find((r) => r.type === 'tool_result' && r.name === 'flaky')
  ...
  test('tool with retry fails after max attempts', async () => {
    let attempts = 0
    const alwaysFailsTool = app.tool({
      name: 'always_fails',
...
describe('maxSteps', () => {
  test('stops after maxSteps iterations', async () => {
    ...
    const { iterations } = await runTest(testAgent({ tools: [loopTool], maxSteps: 2 }), [
      user('Loop forever'),
...
describe('invocation tracking', () => {
  test('creates invocation start and end events with matching IDs', async () => {
    ...
    expect(starts[0].type === 'invocation_start' && starts[0].agentName).toBe('tracked')
    expect(
      starts[0].type === 'invocation_start' &&
        ends[0].type === 'invocation_end' &&
        ends[0].invocationId,
    ).toBe(starts[0].type === 'invocation_start' && starts[0].invocationId)
    expect(ends[0].type === 'invocation_end' && ends[0].reason).toBe('completed')
  })

  test('events include invocation ID', async () => {
    const { events } = await runTest(testAgent(), [user('Test'), model({ text: 'Response' })])
...
describe('structured output', () => {
  ...
  test('uses valid partial result when full parse fails on optional field', async () => {
    const schema = z.object({
      required: z.string(),
      optional: z.number().optional(),
    })
    const agent = testAgent({
      output: { schema, key: 'out' },
    })

    const { session } = await runTest(agent, [
      user('Run'),
      model({
        text: JSON.stringify({ required: 'value', optional: 'not-a-number' }),
      }),
    ])

    expect(session.state.out).toEqual({ required: 'value' })
  })
})

describe('tool state modification', () => {
  test('tool can modify session state', async () => {
    const stateTool = app.tool({
      name: 'set',
...
describe('result.state', () => {
  test('result.state mirrors result.session.state', async () => {
    const stateTool = app.tool({
      name: 'write',
    ...
    const { result } = await runTest(testAgent({ tools: [stateTool] }), [
      user('Write state'),
      model({ toolCalls: [{ name: 'write', args: { key: 'count', value: 7 } }] }),
      model({ text: 'Done' }),
    ])

    expect(result.state.count).toBe(7)
    expect(result.state).toBe(result.session.state)
  })

  test('result.state is available on structured output runs', async () => {
    const agent = testAgent({
      output: { schema: z.object({ name: z.string() }), key: 'item' },
    })

    const { result } = await runTest(agent, [
      user('Get item'),
      model({ text: JSON.stringify({ name: 'Widget' }) }),
    ])

    expect(result.state.item).toEqual({ name: 'Widget' })
  })
})
