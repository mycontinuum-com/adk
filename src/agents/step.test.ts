...
describe('step composition', () => {
  ...
  describe('invocation events', () => {
    test('emits correct invocation events', async () => {
      ...
      const stepEnd = ends.find((e) => e.type === 'invocation_end' && e.agentName === 'my_step')
  ...
  describe('composition with agents', () => {
    ...
    test('step after agent', async () => {
      ...
      const { session } = await runTest(pipeline, [user('Go'), model({ text: 'Agent response' })])
  ...
  describe('orchestration', () => {
    test('can run agents with ctx.run', async () => {
      const innerAgent = testAgent({ name: 'inner_agent' })

      const orchestratingStep = step({
        name: 'orchestrating_step',
        execute: async (ctx) => {
          const result = await ctx.run(innerAgent, 'Hello from step')
          ctx.state.callResult = {
            status: result.status,
            hasOutput: result.output.text !== undefined || result.output.value !== undefined,
          }
        },
      })
      ...
      const { session } = await runTest(pipeline, [user('Go'), model({ text: 'Inner response' })])
