import { runTest, user, model, mockAgent, testAgent, setupAdkMatchers } from '../testing'
...
describe('step routing and signals', () => {
  describe('basic routing', () => {
    ...
    test('routes based on session state', async () => {
      ...
      const { events } = await runTest(gate, [user('Hello'), model({ text: 'Agent A response' })], {
    ...
    test('supports async execute function', async () => {
      ...
      const { status } = await runTest(gate, [user('Hello'), model({ text: 'Response' })])
    ...
    test('conditional skip in sequence', async () => {
      ...
      const conditionalStep = step({
        ...
        execute: (ctx) => {
          ...
          return shouldSkip ? ctx.skip() : mockAgent('middle', { responses: [{ text: 'Middle' }] })
        },
      })
  ...
  describe('state assignment', () => {
    test('direct state assignment sets output value', async () => {
      const gate = step({
        name: 'state_gate',
        execute: (ctx) => {
          ctx.state.result = { cached: true, value: 42 }
        },
      })
    ...
    test('cache gate pattern with state assignment', async () => {
  ...
  describe('signal throw behavior', () => {
    test('signals take effect without explicit return', async () => {
      const gate = step({
        name: 'no_return_gate',
        execute: (ctx) => {
          ctx.skip()
    ...
    test('code after signal is not executed', async () => {
      ...
      await runTest(gate, [user('Hello')])

      expect(afterSignal).toBe(false)
    })
  })
  ...
  describe('invocation events', () => {
    test('emits correct invocation events', async () => {
      ...
      const stepStart = starts.find((e) => e.type === 'invocation_start' && e.agentName === 'gate')
  ...
  describe('real-world patterns', () => {
    ...
    test('feature flag routing', async () => {
      ...
      const { events } = await runTest(featureStep, [user('Go'), model({ text: 'New feature!' })], {
