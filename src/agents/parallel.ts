import type { Parallel, RunResult, StreamEvent, Event } from '../types'
...
export async function* runParallel(
  ...
  async function* execute(): AsyncGenerator<StreamEvent, WorkflowResult<Parallel>> {
    ...
    const yieldedBranches = branches.filter((b) => b.result?.status === 'yielded_tool')
    const completedBranches = branches.filter((b) => b.result?.status === 'completed')
    const errorBranches = branches.filter((b) => b.error !== null || b.result?.status === 'error')
    ...
    if (yieldedBranches.length > 0) {
      const allPendingCalls = yieldedBranches.flatMap((b) => {
        if (b.result?.status === 'yielded_tool') {
          return b.result.yieldedTools
        }
        return []
      })
    ...
    const allCompleted = [...alreadyCompleted, ...completedBranches.map((b) => b.index)]
    const failedBranches = errorBranches.map((b) => ({
      index: b.index,
      error: b.error ?? (b.result?.status === 'error' ? b.result.error : 'Unknown error'),
    }))

    if (runnable.minSuccessful && allCompleted.length < runnable.minSuccessful) {
      const failedSummary = failedBranches.map((f) => `Branch ${f.index}: ${f.error}`).join('; ')
