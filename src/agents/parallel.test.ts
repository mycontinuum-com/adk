...
import { runTest, user, mockAgent, testAgent, setupAdkMatchers, MockAdapter } from '../testing'
...
describe('parallel composition', () => {
  ...
  describe('custom merge', () => {
    test('custom merge function can add events', async () => {
      ...
      expect(
        [...events].some(
          (e) => e.type === 'assistant' && (e as { text: string }).text === 'Merged Summary',
        ),
      ).toBe(true)
    })
  })
  ...
  describe('concurrent state modifications', () => {
    test('state changes from parallel branches are captured', async () => {
      ...
      const branchWithTool = (name: string, _key: string, _value: string) =>
        testAgent({
          name,
          tools: [setTool],
      ...
      const adapter = new MockAdapter({
        responses: [
          {
            toolCalls: [{ name: 'set_state', args: { key: 'b1', value: 'v1' } }],
          },
          { text: 'Done 1' },
          {
            toolCalls: [{ name: 'set_state', args: { key: 'b2', value: 'v2' } }],
          },
          { text: 'Done 2' },
        ],
      })
      ...
      const fanout = parallel({
        name: 'state_test',
        runnables: [branchWithTool('branch1', 'b1', 'v1'), branchWithTool('branch2', 'b2', 'v2')],
      })

      const session = new BaseSession('test-app', { id: `test-${Date.now()}` })
      session.input.message('Set states')

      const _result = await runner.run(fanout, session)
  ...
  describe('invocation tracking', () => {
    test('creates invocation events with parent-child relationship', async () => {
      ...
      const parent = starts.find((e) => e.type === 'invocation_start' && e.agentName === 'tracked')
      const branches = starts.filter(
        (e) => e.type === 'invocation_start' && e.agentName.startsWith('branch_'),
      )
